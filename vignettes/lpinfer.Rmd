...
The `replace` parameter is used to indicate whether the function 
samples the data with or without replacement. Currently there are three 
options available:

| `replace`| `phi` | Meaning |
| --------------- | :-------------- | :-------------- |
|`FALSE`| Any number between 0 and 1 | Subsample |
|`TRUE`| Equals to 1 | Bootstrap |
|`TRUE`| Any nonnegative number other than 1 | *m* out of *n* bootstrap |
...
the bootstrap and *m* out of *n* boostrap procedures. They are illustrated
as follows.

The following is an exmaple of performing a bootstrapping procedure:

``` {r, subsample_eg3, eval = TRUE}
set.seed(1)
