...
`subsample` command can perform the bootstrap and *m* out of *n* boostrap 
...
estimates of the cone coponent in the test statistics. Users can pass 
multiple `lambda`s in this argument. In addition, we also provide a 
data-driven approach to choose the `lambda` parameter. The details can be found [here](#data-driven-lambda).
...
refer to the instructions from the `future` pakage for more details.
