...
linear and quadratic programming. The `linearprog` package can be used to
construct the *p*-value and confidence interval for the missing data problem.
...
The `many_invertci` function is a wrapper for the `invertci` function where the user 
...
```{r, many_invertci_syntax, eval = FALSE}
...
* `progress_one` refers to the boolean variable for whether the result messages
should be displayed in running the function `invertci`.
* `progress_many` refers to the boolean variable for whether the result 
messages should be displayed in running the function `many_invertci`.
* The rest of the arguments are the same as the function `invertci` and they can 
be found [here](#invertci_syntax).

#### Output

The following shows a sample output of the function `invertci_many` that is used to 
the confidence intervals for the test `dkqs` with significance level 
0.01, 0.05 and 0.1.
